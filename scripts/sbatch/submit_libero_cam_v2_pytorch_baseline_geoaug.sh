...
if [[ "${RESUME}" == "true" ]]; then
  TARGET_SCRIPT="${REPO_ROOT}/scripts/sbatch/train_pi0_libero_cam_v2_pytorch_baseline_geoaug_resume.sbatch"
else
  TARGET_SCRIPT="${REPO_ROOT}/scripts/sbatch/train_pi0_libero_cam_v2_pytorch_baseline_geoaug.sbatch"
fi
...
echo "Submitting PyTorch baseline-geoaug training"
...
echo "exp name: ${EXP_NAME:-pi0_libero_cam_v2_pytorch_baseline_geoaug_v1}"
