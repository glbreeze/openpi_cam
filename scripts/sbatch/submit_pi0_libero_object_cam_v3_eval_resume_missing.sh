...
has_summary_for_step() {
  local log_root="$1"
  local step="$2"
  local summary_glob="${log_root}/step_${step}/summary_${step}_"'*.json'

  compgen -G "${summary_glob}" > /dev/null
}
...
TARGET_SCRIPT=${TARGET_SCRIPT:-${REPO_ROOT}/scripts/sbatch/eval_pi0_libero_object_cam_v3_pytorch_baseline_full.sbatch}
...
LOG_ROOT=${LOG_ROOT:-${REPO_ROOT}/log/libero_eval/${EXP_NAME}/full}
...
mapfile -t all_steps < <(collect_steps "${CHECKPOINT_ROOT}" "${MIN_STEP}" "${MAX_STEP}" "${STEP_STRIDE}")
if ((${#all_steps[@]} == 0)); then
  echo "No checkpoint steps found under ${CHECKPOINT_ROOT}" >&2
  exit 1
fi

declare -A force_step_map=()
if [[ -n "${FORCE_STEPS}" ]]; then
  for step in ${FORCE_STEPS//,/ }; do
    [[ "${step}" =~ ^[0-9]+$ ]] || {
      echo "Invalid step in FORCE_STEPS: ${step}" >&2
      exit 1
    }
    force_step_map["${step}"]=1
  done
fi
...
for step in "${all_steps[@]}"; do
  if [[ -n "${force_step_map[${step}]:-}" ]]; then
    steps_to_submit+=("${step}")
    continue
  fi
  if has_summary_for_step "${LOG_ROOT}" "${step}"; then
...
echo "Resume-missing LIBERO eval sweep"
echo "repo root: ${REPO_ROOT}"
echo "target script: ${TARGET_SCRIPT}"
echo "config: ${CONFIG_NAME}"
echo "exp name: ${EXP_NAME}"
echo "checkpoint root: ${CHECKPOINT_ROOT}"
echo "all steps: ${all_steps[*]}"
echo "completed steps: ${completed_steps[*]:-<none>}"
echo "steps to submit: ${steps_to_submit[*]:-<none>}"
...
if ((${#steps_to_submit[@]} == 0)); then
  echo "Nothing to submit."
  exit 0
fi

if [[ "${DRY_RUN}" == "1" ]]; then
  echo "Dry run requested. Exiting without submitting."
  exit 0
fi

manifest="${MANIFEST_DIR}/resume_missing_manifest_$(date +%Y%m%d_%H%M%S).tsv"
printf 'step\tjob_id\tlog_root\n' > "${manifest}"

for idx in "${!steps_to_submit[@]}"; do
  step="${steps_to_submit[$idx]}"
  port=$((BASE_PORT + idx))
  ...
  SBATCH_ARGS+=(--job-name="pi0_eval_${step}")
  SBATCH_ARGS+=(--output="${step_log_root}/slurm-%j.out" --error="${step_log_root}/slurm-%j.err")

  job_id=$(
    sbatch --parsable "${SBATCH_ARGS[@]}" \
      --export=ALL,CONFIG_NAME="${CONFIG_NAME}",EXP_NAME="${EXP_NAME}",CHECKPOINT_ROOT="${CHECKPOINT_ROOT}",CHECKPOINT_STEP="${step}",SUITE_NAME="${SUITE_NAME}",NUM_TRIALS_PER_TASK="${NUM_TRIALS_PER_TASK}",TASK_ID_START="${TASK_ID_START}",TASK_ID_END="${TASK_ID_END}",SERVER_PORT="${port}",LOG_ROOT="${step_log_root}",MODEL_NAME="${EXP_NAME}_ckpt${step}" \
      "${TARGET_SCRIPT}"
  )
