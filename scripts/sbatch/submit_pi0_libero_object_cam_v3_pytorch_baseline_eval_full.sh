...
TARGET_SCRIPT="${REPO_ROOT}/scripts/sbatch/eval_pi0_libero_object_cam_v3_pytorch_baseline_full.sbatch"
...
for idx in "${!steps[@]}"; do
  ...
  job_id=$(
    sbatch --parsable "${SBATCH_ARGS[@]}" \
      --export=ALL,CONFIG_NAME=pi0_libero_object_pytorch_baseline,EXP_NAME="${EXP_NAME}",CHECKPOINT_ROOT="${CHECKPOINT_ROOT}",CHECKPOINT_STEP="${step}",SUITE_NAME="${SUITE_NAME}",NUM_TRIALS_PER_TASK="${NUM_TRIALS_PER_TASK}",TASK_ID_START="${TASK_ID_START}",TASK_ID_END="${TASK_ID_END}",SERVER_PORT="${port}",LOG_ROOT="${step_log_root}",MODEL_NAME="${EXP_NAME}_ckpt${step}" \
      "${TARGET_SCRIPT}"
  )
