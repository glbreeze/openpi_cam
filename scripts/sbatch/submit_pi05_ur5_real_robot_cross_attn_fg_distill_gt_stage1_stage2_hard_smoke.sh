...
TARGET_SCRIPT="${REPO_ROOT}/scripts/sbatch/train_pi05_ur5_real_robot_cross_attn_fg_distill_gt_stage1_stage2_hard_smoke.sbatch"
...
echo "Submitting pi0.5 UR5 real-robot fg + hard GT Stage 1 + Stage 2 smoke"
...
echo "stage1 exp: ${STAGE1_EXP_NAME:-pi05_ur5_real_robot_cross_attn_fg_distill_gt_hard_stage1_smoke}"
echo "stage2 config: ${STAGE2_CONFIG_NAME:-pi05_ur5_real_robot_pytorch_cross_attn_fg_distill_gt_stage2_hard}"
echo "stage2 exp: ${STAGE2_EXP_NAME:-pi05_ur5_real_robot_cross_attn_fg_distill_gt_hard_stage2_smoke}"
...
echo "checks: GT root only, context_left/wrist_right cache, GT base/left_wrist layout, 224 targets, point-target passthrough, stage2 handoff, nonzero GT aux loss"
