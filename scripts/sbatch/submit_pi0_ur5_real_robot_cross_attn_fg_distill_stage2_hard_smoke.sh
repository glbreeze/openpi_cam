...
TARGET_SCRIPT="${REPO_ROOT}/scripts/sbatch/train_pi0_ur5_real_robot_cross_attn_fg_distill_stage2_hard_smoke.sbatch"
...
echo "Submitting UR5 real-robot fg + hard Stage 2 smoke test"
...
echo "stage1 checkpoint root: ${STAGE1_CHECKPOINT_ROOT:-/scratch/${USER}/tmp/openpi_cam/checkpoints/pi0_ur5_real_robot_pytorch_cross_attn_fg_distill_stage1_hard/pi0_ur5_real_robot_cross_attn_fg_distill_hard_stage1_smoke}"
echo "stage1 checkpoint step: ${STAGE1_CHECKPOINT_STEP:-20}"
echo "checkpoint base dir: ${CHECKPOINT_BASE_DIR:-/scratch/${USER}/tmp/openpi_cam/checkpoints}"
echo "exp name: ${EXP_NAME:-pi0_ur5_real_robot_cross_attn_fg_distill_hard_stage2_smoke}"
...
echo "default num train steps: ${NUM_TRAIN_STEPS:-20}"
echo "default save interval: ${SAVE_INTERVAL:-10}"
