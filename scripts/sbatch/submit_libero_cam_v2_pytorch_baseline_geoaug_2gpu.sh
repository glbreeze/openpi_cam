...
if [[ "${RESUME}" == "true" ]]; then
  TARGET_SCRIPT="${REPO_ROOT}/scripts/sbatch/train_pi0_libero_cam_v2_pytorch_baseline_geoaug_2gpu_resume.sbatch"
else
  TARGET_SCRIPT="${REPO_ROOT}/scripts/sbatch/train_pi0_libero_cam_v2_pytorch_baseline_geoaug_2gpu.sbatch"
fi
...
echo "Submitting PyTorch baseline-geoaug 2-GPU training"
...
echo "exp name: ${EXP_NAME:-pi0_libero_cam_v2_pytorch_baseline_geoaug_2gpu_v1}"
