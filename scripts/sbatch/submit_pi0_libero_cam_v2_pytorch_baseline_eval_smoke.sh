...
LOG_ROOT=${LOG_ROOT:-${REPO_ROOT}/log/libero_eval/${EXP_NAME}/${SUITE_NAME}/smoke}
...
sbatch "${SBATCH_ARGS[@]}" \
  --export=ALL,CONFIG_NAME=pi0_libero_cam_v2_pytorch_baseline,EXP_NAME="${EXP_NAME}",CHECKPOINT_ROOT="${CHECKPOINT_ROOT}",CHECKPOINT_STEP="${CHECKPOINT_STEP}",SUITE_NAME="${SUITE_NAME}",NUM_TRIALS_PER_TASK="${NUM_TRIALS_PER_TASK}",TASK_ID_START="${TASK_ID_START}",TASK_ID_END="${TASK_ID_END}",SERVER_PORT="${SERVER_PORT}",LOG_ROOT="${LOG_ROOT}" \
  "${TARGET_SCRIPT}"
