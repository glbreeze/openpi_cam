...
def _resize_image(img: np.ndarray, target_hw: int) -> np.ndarray:
    ...
    Pass-through orientation: Sapien renders images in OpenCV (y-down) convention,
    which is already what the model and the official RoboTwin pi0 eval client
    use. No 180-deg flip applied (unlike LIBERO/MuJoCo which needs one to undo
    OpenGL y-up). K stored alongside is the natural Sapien K with positive fx.
    """
