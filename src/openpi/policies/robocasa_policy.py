...
class RobocasaInputs(transforms.DataTransformFn):
    ...
    Image-to-slot mapping:
        ...
        agentview_right-> right_wrist_0_rgb  (optional; padded if absent)
    ...
    def __call__(self, data: dict) -> dict:
        if "images" in data:
            ...
            missing = [k for k, v in source_images.items() if v is None and k != "agentview_right"]
            if missing:
        ...
        inputs = {
            ...
            "image_mask": {
                ...
                # RoboCasa24 all24_human_camaware only ships left + wrist. Pad
                # the right slot when absent so baseline training can reuse the
                # same policy transform as the 3-camera RoboCasa365 path.
                "right_wrist_0_rgb": np.bool_(has_right_image),
            },
        }
