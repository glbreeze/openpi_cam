...
Vision-language-action (VLA) models have reframed language-conditioned manipulation as sequence modeling over images, text, and robot actions. RT-1 demonstrated that transformer policies can scale across diverse robot data~\citep{brohan2023rt1}, while RT-2 showed that Internet-scale vision-language pretraining can transfer semantic knowledge to robot control~\citep{zitkovich2023rt2}. OpenVLA and $\pi_0$ further make this paradigm practical through open VLA checkpoints and continuous-action flow matching~\citep{kim2024openvla,black2025pi0}. These models provide strong semantic and task-level priors, but their visual interfaces are still largely inherited from 2D vision-language backbones. Camera geometry, when available, is typically left for the policy to infer implicitly from robot demonstrations. Our work is complementary: we keep the pretrained VLA objective and action representation intact, and instead study how calibrated camera geometry can be injected into the visual-token interface.

\paragraph{Structured 3D representations for manipulation.}
Many manipulation policies improve spatial precision by constructing explicit 3D scene representations. PerAct voxelizes RGB-D observations and predicts actions with a transformer over a 3D workspace~\citep{shridhar2023peract}. RVT improves efficiency by aggregating rendered multi-view observations while preserving 3D cues~\citep{goyal2023rvt}. Act3D lifts 2D visual features into a 3D feature field and performs coarse-to-fine action selection in 3D~\citep{gervet2023act3d}, while PolarNet learns language-conditioned policies directly from 3D point clouds~\citep{chen2023polarnet}. These methods demonstrate the value of geometric structure for manipulation, especially in settings requiring accurate end-effector placement. Our goal is different: rather than replacing the VLA observation interface with voxels, point clouds, or explicit 3D action maps, we expose calibrated geometry inside the visual-token stream of an existing VLA.

\paragraph{Spatially aware VLAs.}
Recent work has begun to connect VLA policies more directly with 3D or spatial representations. 3D-VLA couples action prediction with 3D world modeling~\citep{zhen2024threedvla}. SpatialVLA introduces spatial encodings and action discretizations designed to improve cross-robot spatial transfer~\citep{qu2025spatialvla}. 3DS-VLA builds a spatially aware VLA using 3D representations for robust multi-task manipulation~\citep{li2025threedsvla}. These systems highlight an important trend: stronger spatial representations can improve the precision and robustness of generalist robot policies. Our method occupies a lighter-weight point in this design space: it uses calibrated camera parameters and teacher-supervised visual tokens to make an RGB-based VLA more geometry-aware, without requiring point clouds, reconstructed scenes, or a new action representation at test time.

\paragraph{Camera-aware representations and geometry distillation.}
Multi-camera robot platforms often provide calibrated intrinsics and extrinsics, yet policies frequently process each camera stream as an ordinary image. Camera-aware positional encodings offer a direct way to expose this structure to attention layers. Cameras as Relative Positional Encoding formulates camera geometry as a projective positional signal for transformer attention, allowing correspondences across views to be represented through the camera model rather than only through learned appearance statistics~\citep{li2025cameras}. We adapt this idea to VLA manipulation by combining projective positional encoding with intrinsic-conditioned ray embeddings and cross-view fusion.

Supervision is the second challenge: learning geometry-aware tokens with dense metric labels would normally require annotation, depth sensors, or reconstruction. We instead distill full-resolution ray-coordinate and log-depth targets from a visual geometry teacher based on $\pi^3$, which learns permutation-equivariant visual geometry from images~\citep{wang2025pi3}. The auxiliary objective shapes intermediate visual features during training; the teacher is not used by the policy at test time.
