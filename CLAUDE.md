...
The repo also carries a camera-aware Pi3X distillation pipeline on top of Pi0 (LIBERO and RoboCasa), with its own caching scripts (`scripts/cache_pi3x_targets.py`, `scripts/cache_libero_gt_point_targets.py`, `scripts/cache_robocasa24_pi3x_targets.py`, `scripts/cache_robocasa24_gt_point_targets.py`, `scripts/cache_robocasa24_cam_matrices.py`), dataset converters (`scripts/convert_robocasa24_to_camaware_lerobot.py`, `scripts/merge_robocasa24_lerobot.py`), HPC SLURM templates under `scripts/sbatch/`, env helpers under `scripts/env/` (notably `activate_env.sh`, which sets `HF_LEROBOT_HOME`, `OPENPI_PI0_BASE_DIR`, etc. from `GEO_ROOT`), and one-off utilities under `scripts/utils/`. See [docs/pi3x_pi0_system_architecture_and_results.md](docs/pi3x_pi0_system_architecture_and_results.md) for the architecture overview; [docs/pi3x_distillation.md](docs/pi3x_distillation.md) and [docs/pi3x_distillation_hpc.md](docs/pi3x_distillation_hpc.md) for LIBERO recipes; [docs/robocasa24_plan.md](docs/robocasa24_plan.md) for RoboCasa; and [docs/real_robot_ur5_training.md](docs/real_robot_ur5_training.md) for the UR5 baseline.
