...
- `policies/` — input/output transforms that map a specific robot/dataset (`aloha_policy.py`, `droid_policy.py`, `libero_policy.py`, `robotwin_policy.py`, `real_robot_policy.py`) to the model's observation/action format. `policy.py` wraps a trained model for inference; `policy_config.create_trained_policy` is the main entry point and autodetects JAX vs. PyTorch checkpoints.
- `training/` — `config.py` is the central registry: it defines `TrainConfig`, `DataConfig`, `AssetsConfig`, and the `_CONFIGS` list of named recipes (e.g. `pi05_libero`, `pi0_aloha_sim`, `debug`, plus the `pi0_libero_cam_pytorch_prope_ray_view_distill_fullres_*` and `pi0_robotwin_cam_prope_ray_view_*` Pi3X recipes). `data_loader.py` builds LeRobot-backed loaders; `weight_loaders.py` loads base-model weights (including `pytorch_weight_path` for PyTorch); `optimizer.py`, `sharding.py` (FSDP mesh), `checkpoints.py`, and `droid_rlds_dataset.py` are used by both trainers. `misc/` holds environment-specific overrides.
...
The repo also carries a camera-aware Pi3X distillation pipeline on top of Pi0 (LIBERO and RoboTwin), with its own caching scripts (`scripts/cache_pi3x_targets.py`, `scripts/cache_libero_gt_point_targets.py`, `scripts/cache_robotwin_gt_point_targets.py`), dataset converters (`scripts/convert_robotwin_cam_to_lerobot.py`, `scripts/convert_aloha_data_to_lerobot_robotwin_official.py`, `scripts/robotwin_official_process_data.py`), HPC SLURM templates under `scripts/sbatch/`, env helpers under `scripts/env/` (notably `activate_env.sh`, which sets `HF_LEROBOT_HOME`, `OPENPI_PI0_BASE_DIR`, etc. from `GEO_ROOT`), RoboTwin eval glue under `scripts/robotwin_eval_policies/` and `scripts/robotwin_runtime/`, and one-off utilities under `scripts/utils/`. See [docs/pi3x_pi0_system_architecture_and_results.md](docs/pi3x_pi0_system_architecture_and_results.md) for the architecture overview; [docs/pi3x_distillation.md](docs/pi3x_distillation.md) and [docs/pi3x_distillation_hpc.md](docs/pi3x_distillation_hpc.md) for LIBERO recipes; [docs/robotwin_cam_pipeline.md](docs/robotwin_cam_pipeline.md) and [docs/robotwin_5tasks_hpc_runbook.md](docs/robotwin_5tasks_hpc_runbook.md) for RoboTwin; [docs/real_robot_ur5_training.md](docs/real_robot_ur5_training.md) for the UR5 baseline; and [scripts/HPC_ROBOTWIN_PI0_RUNBOOK.md](scripts/HPC_ROBOTWIN_PI0_RUNBOOK.md) for the upstream-LeRobot RoboTwin Pi0 path.
